...
var wasmBinaryFile = 'ffprobe-wasm.wasm';
  ...
  loadWasmModuleToWorker: function(worker, onFinishedLoading) {
    worker.onmessage = function(e) {
      ...
      if(cmd === 'processQueuedMainThreadWork') {
        ...
        var detached =
          worker.pthread &&
          Atomics.load(HEAPU32, (worker.pthread.thread + 64) >> 2);
        if(detached) {
          PThread.returnWorkerToPool(worker);
        }
...
function ___sys_rename(old_path, new_path) {
  if(ENVIRONMENT_IS_PTHREAD) {
    return _emscripten_proxy_to_main_thread_js(8, 1, old_path, new_path);
  }
  try {
...
function _fd_fdstat_get(fd, pbuf) {
  if(ENVIRONMENT_IS_PTHREAD) {
    return _emscripten_proxy_to_main_thread_js(16, 1, fd, pbuf);
  }
  try {
    var stream = SYSCALLS.getStreamFromFD(fd);
    var type = stream.tty
...
function _tzset() {
  if(ENVIRONMENT_IS_PTHREAD) {
    return _emscripten_proxy_to_main_thread_js(20, 1);
  }
  if(_tzset.called) {
    return;
  }
...
function _sysconf(name) {
  if(ENVIRONMENT_IS_PTHREAD) {
    return _emscripten_proxy_to_main_thread_js(21, 1, name);
  }
  switch (name) {
  case 30:
    return 16384;
...
var asmLibraryArg = {
  ...
  R: __emscripten_notify_thread_queue,
  ia: __emval_decref,
  ja: __emval_incref,
  ...
  u: _emscripten_set_current_thread_status,
  Q: _emscripten_webgl_create_context,
  U: _environ_get,
...
var __get_tzname = (Module['__get_tzname'] = function() {
  return (__get_tzname = Module['__get_tzname'] = Module['asm']['xa']).apply(
    null,
    arguments
  );
});
...
var _memalign = (Module['_memalign'] = function() {
  return (_memalign = Module['_memalign'] = Module['asm']['Pa']).apply(
    null,
    arguments
  );
});
...
var dynCall_iiiji = (Module['dynCall_iiiji'] = function() {
  return (dynCall_iiiji = Module['dynCall_iiiji'] = Module['asm']['Ra']).apply(
    null,
    arguments
  );
});
var _ff_h264_cabac_tables = (Module['_ff_h264_cabac_tables'] = 106421);
